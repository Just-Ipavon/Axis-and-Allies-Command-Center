...
function hashPassword(password) {
    ...
    const hash = crypto.pbkdf2Sync(password + PEPPER, salt, 1000, 64, 'sha512').toString('hex');
    return `${salt}:${hash}`;
}
...
function verifyPassword(password, storedHash) {
    ...
    const verifyHash = crypto.pbkdf2Sync(password + PEPPER, salt, 1000, 64, 'sha512').toString('hex');
